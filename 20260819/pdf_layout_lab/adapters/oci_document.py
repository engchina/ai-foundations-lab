...
class OciDocumentAdapter:
    ...
    def analyze(self, context: AnalysisContext) -> list[LayoutRecord]:
        ...
        for batch_index, batch_pages in enumerate(_chunk_pages(context.pages, OCI_SYNC_MAX_PAGES), start=1):
            subset_path = _write_subset_pdf(context.pdf_path, batch_pages, context.run_dir / "oci", batch_index)
            page_number_map = {index + 1: page.page for index, page in enumerate(batch_pages)}
            ...
            details_kwargs = dict(
                ...
                document=oci.ai_document.models.InlineDocumentDetails(
                    source="INLINE",
                    data=encoded,
                    page_range=_page_ranges_from_pages(page_number_map.keys()),
                ),
                features=[
